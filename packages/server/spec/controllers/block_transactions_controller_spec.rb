...
describe BlockTransactionsController, type: :controller do
  describe 'GET #index' do
    ...
    context 'mined' do
      context 'list not empty' do
        ...
        it { expect(body[0]['transaction_hash']).to eq transactions[1].transaction_hash }
        it { expect(body[1]['transaction_hash']).to eq transactions[2].transaction_hash }
        it { expect(body[2]['transaction_hash']).to eq transactions[0].transaction_hash }
  ...
  describe 'POST #save' do
    ...
    context 'transaction' do
      let(:token) { create :token, address_hash: '0x0A00000000000000000000000000000000000222' }
      let(:params) do
        {
          block_transaction: {
