...
describe TokenPrice, type: :model do
  describe 'association' do
    it { is_expected.to belong_to :token }
  end

  describe 'validation' do
    subject { create :token_price }
  ...
  describe 'method' do
    describe '#before_datetime' do
      let(:prices) do
        [
          create(:token_price, datetime: 4.hours.ago),
          create(:token_price, datetime: 2.hours.ago),
          create(:token_price, datetime: 1.hours.ago),
          create(:token_price, datetime: 5.hours.ago)
        ]
      end
      ...
      context 'when previous price' do
        subject { TokenPrice.before_datetime 3.hours.ago }

        it { expect(subject.first).to eq prices[0] }
      end

      context 'when no previous price' do
        subject { TokenPrice.before_datetime 6.hours.ago }
